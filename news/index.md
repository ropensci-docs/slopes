...
- Migrated from the deprecated `raster` package to `terra` (closes
  ...
  a hard dependency (`Imports`); `raster` is no longer required or
  suggested.
...
- Fixed
  ...
  hardcoded testing logic, allowing it to correctly return any number of
  requested colors for the “Green-Brown” palette.
- Substantially improved overall package test coverage (from ~59% to
  over 84%) by adding test wrappers for plot functions and matrix
